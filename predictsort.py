...
def analyze_video(job: Job, loop: asyncio.AbstractEventLoop):
    ...
    print("Starting tracking with new model and shot detection...")
    while cap.isOpened():
        ...
        if len(made_shot_boxes) > 0 and shot_cooldown == 0:
            ...
            shot_x = int((shot_box[0] + shot_box[2]) / 2)
            shot_y = int((shot_box[1] + shot_box[3]) / 2)

            # Draw a circle for the "made" event
            ...
            if player_data_this_frame:
                ...
                shot_data.append({
                    "frame": frame_idx, "x": shot_x, "y": shot_y, 
        ...
        other_objects = result.boxes[~np.isin(result.boxes.cls.cpu(), [1, 2])] # Get all other objects
        for box in other_objects:
            x1, y1, x2, y2 = map(int, box.xyxy[0])
            cls = int(box.cls[0])
            if cls == 0: # basketball
                cv2.circle(annotated_frame, (int((x1+x2)/2), int((y1+y2)/2)), 7, (0, 165, 255), -1) # Orange circle
            elif cls == 3: # rim
