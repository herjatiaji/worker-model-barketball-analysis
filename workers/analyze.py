...
def analyze_video(job: Job, loop: asyncio.AbstractEventLoop):
    ...
    s3_video_key = f"results/{job.data.get('id')}_{uuid.uuid4()}.mp4"
    s3_thumbnail_key = f"thumbnails/{job.data.get('id')}_{uuid.uuid4()}.jpg"
    ...
    try:
        ...
        with engine.begin() as conn:
            ...
            print(f"Updating video record ID: {job.data.get('id')}")
            video_stmt = update(videos).where(
                videos.c.id == job.data.get('id')
            ).values(
